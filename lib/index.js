  ...
  fs.readFile(path, function(err, data) {
    ...
    async.each(events, function(event, cb) {
      if(event.type === "VEVENT") {
        ...
        if(eventDocument.metadata.description) {
          // remove the default description
          eventDocument.metadata.description = eventDocument.metadata.description.replace(/[^\n]+https?:\/\/www\.google\.com\/calendar\/event\?action=VIEW.*/, '');

          if(eventDocument.metadata.description.length === 0) {
