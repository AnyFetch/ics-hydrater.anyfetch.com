...
module.exports = function(path, document, changes, finalCb) {
  fs.readFile(path, function(err, data) {
    ...
    var anyfetch = new Anyfetch(null, null, finalCb.apiUrl);
    anyfetch.setAccessToken(document.access_token);
    ...
    async.each(events, function(event, cb) {
      if(event.type === "VEVENT") {
        ...
        anyfetch.sendDocument(eventDocument, function(err) {
          if(err) {
            cb(err);
          }
    ...
    function(err) {
      ...
      anyfetch.deleteDocument(document.identifier, function(err) {
        finalCb(err, {});
      });
    });
  });
